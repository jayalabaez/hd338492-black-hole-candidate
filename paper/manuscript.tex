...
$\fmass = 2.39\,\Msun$ --- above the Chandrasekhar limit from
...
inclination priors gives $P(\text{BH}) = 99.7$~per~cent.  No luminous
companion is detected in any photometric band: a main-sequence star
of the required mass would contribute $>$\,943~per~cent of the system
flux, exceeding the detection threshold by a factor of $943$.  ROSAT
...
6.5 -- 21.2\,$\Msun$) and $P(\text{BH}) = 99.7$~per~cent.  Even at
...
\begin{enumerate}
\item The spectroscopic mass function $\fmass = 2.39\,\Msun$ exceeds
...
\item No luminous companion is detected: a MS star of the required
  ...
  factor of 943.  The companion is dark.
  
\item A Monte~Carlo mass posterior with isotropic inclination
  priors gives $P(\text{BH}) = 99.7$~per~cent.
...
$P(\text{BH})$         & Confirmed & Confirmed & Confirmed & 99.7\% \\
