...
spectral anchor (B8--A2), and extinction ($R_V = 2.5$--$4.0$).  No luminous companion is detected: a main-sequence
...
We fit the dereddened photometry (Gaia + 2MASS + WISE) using both a
pure blackbody and an ATLAS9-corrected model that accounts for line
blanketing via wavelength- and $T_\mathrm{eff}$-dependent correction
factors interpolated from the \citet{CastelliKurucz2004} synthetic
photometry grids ($\log g = 4.0$, $[\text{M/H}] = 0.0$).  In both
cases, $T_\mathrm{eff}$ is fixed from the spectral anchor and $A_V$
is derived from the colour excess $E(G_\mathrm{BP} - G_\mathrm{RP})$;
only the overall flux scale is a free parameter.  The
ATLAS9-corrected SED yields $\chi^2_\nu = 10.9$ for the fiducial B9
anchor, a 20~per~cent improvement over the pure-blackbody fit
($\chi^2_\nu = 12.5$), with the correction strongest in the Gaia
$G_\mathrm{BP}$ band where Balmer-line blanketing is most
significant.  For comparison, the uncorrected (pre-dereddening)
SED gives $\chi^2 \approx 285$, a factor of $\sim$26 worse.
Further improvement is obtained with $R_V = 3.5$
($\chi^2_\nu = 5.8$), suggesting a mildly non-standard extinction
law along the sightline at $b = 3.0\degr$.  The absence of a
secondary SED component sets a stringent upper limit on the
luminosity of any hidden companion.
...
relations \citep{Eker2018} and Planck spectral ratios corrected by
the \citet{CastelliKurucz2004} model-atmosphere factors.  A companion
...
\item \textbf{The SED analysis has been upgraded} from a pure
  blackbody to an ATLAS9-corrected model
  \citep{CastelliKurucz2004}, reducing $\chi^2_\nu$ from 12.5
  to 10.9 (and to 5.8 with $R_V = 3.5$).  However, further
  refinement with full synthetic spectra (e.g.\ PHOENIX) convolved
  with filter profiles could tighten constraints on any hidden
  secondary.
...
\item \textbf{The $P(\text{BH}) = 99.7$~per~cent value is
  ...
  An expanded sensitivity analysis spanning B8, B9, A0, and A2
  spectral anchors and $R_V \in \{2.5, 3.1, 3.5, 4.0\}$ shows
  that $P(\text{BH})$ ranges from $\sim$88 to $>$\,99~per~cent
  under all tested configurations --- always high, but not
...
HD~338492 is a high-mass dark companion candidate strongly consistent
with a dormant black hole that warrants urgent follow-up.  We emphasise that the gap between
...
Specifically: \texttt{02b\_fit\_sed\_atlas.py} produces
the ATLAS9-corrected SED analysis;
\texttt{03\_compute\_mass\_posterior.py}
produces Fig.~\ref{fig:posterior};
\texttt{04\_companion\_exclusion.py} produces
Fig.~\ref{fig:companion};
\texttt{07b\_sensitivity\_expanded.py} provides the expanded
anchor and $R_V$ sensitivity analysis;
\texttt{08\_archival\_checks.py} carries out the
variability, literature, and contamination audits (C14--C16);
and \texttt{06\_make\_figures.py}
...
Stripped He star & Disfavoured  & UV unconstrained (no GALEX) \\
...
satisfied).  The system scores 9P/5O/2F across 16 checks ---
a BH candidate pending follow-up to close the
